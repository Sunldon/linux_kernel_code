		...
		unsigned long nsect, char *buffer, int write)
{
	...
	if (write){
		...
		//printk(KERN_ALERT"vmem_disk_transfer__write:%ld\r\n",nbytes);
	}
	else{
		memcpy(buffer, dev->data + offset, nbytes);
		//printk(KERN_ALERT"vmem_disk_transfer__read:%ld\r\n",nbytes);
	}
}
...
static int vmem_disk_xfer_bio(struct vmem_disk_dev *dev, struct bio *bio)
{
	...
	bio_for_each_segment(bvec, bio, i) {
		char *buffer = __bio_kmap_atomic(bio, i,5);/*page_address+page_offset*/
		
		printk(KERN_ALERT"vmem_disk_xfer_bio vcnt:%ld idx:%ld, sector:%ld offset:%ld bv_len:%ld \r\n",
			bio->bi_vcnt,bio->bi_idx,bio->bi_sector,bio->bi_io_vec->bv_offset,bio->bi_io_vec->bv_len);
		
		vmem_disk_transfer(dev, sector, bio_cur_bytes(bio) >> 9,
			buffer, bio_data_dir(bio) == WRITE);
		sector += bio_cur_bytes(bio) >> 9;
		__bio_kunmap_atomic(buffer,5);
	}
	return 0;
}
...
static void vmem_disk_request(struct request_queue *q)
{
	...
	while ((req = blk_peek_request(q)) != NULL) {
		...
		printk(KERN_ALERT"vmem_disk_request disk_name:%s \r\n",req->rq_disk->disk_name);
