...
static int force_cleanup_module(char *del_mod_name)
{
    ...
    {
        printk(KERN_EMERG"[%s] module %s not found\n", THIS_MODULE->name, del_mod_name);
        return -1;
    }
    else
    {
        printk(KERN_EMERG"[before] name:%s, state:%d, refcnt:%u\n",
    ...
    if (!list_empty(&mod->source_list))
    {
        ...
        {
            printk(KERN_EMERG"[relate]:%s\n", relate->name);
        }
    }
    else
    {
        printk(KERN_EMERG"No modules depond on %s...\n", del_mod_name);
    }
    ...
    {
        local_set((local_t*)per_cpu_ptr(&(mod->refptr->decs), cpu), 0);
		local_set((local_t*)per_cpu_ptr(&(mod->refptr->incs), cpu), 0);
        //local_set(__module_ref_addr(mod, cpu), 0);
        //per_cpu_ptr(mod->refptr, cpu)->decs;
        //module_put(mod);
    }
    atomic_set(&mod->refptr->decs, 0);
	atomic_set(&mod->refptr->incs, 0);
    ...
    {
        printk(KERN_EMERG"module %s don't have exit function...\n", mod->name);
    }
    else
    {
        printk(KERN_EMERG"module %s exit function address %p\n", mod->name, origin_exit_addr);
    }

    mod->exit = force_replace_exit_module_function;
    printk(KERN_EMERG"replace module %s exit function address (%p -=> %p)\n", mod->name, origin_exit_addr, mod->exit);
#endif

    printk(KERN_EMERG"[after] name:%s, state:%d, refcnt:%u\n",
            mod->name, mod->state, module_refcount(mod));
...
{
    printk(KERN_EMERG"=======name : %s, state : %d EXIT=======\n", THIS_MODULE->name, THIS_MODULE->state);
}
